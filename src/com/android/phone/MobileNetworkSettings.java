...
public class MobileNetworkSettings extends PreferenceActivity
    ...
    protected void onResume() {
        ...
        if (TelephonyConstants.IS_DSDS) {
            int settingsNetworkMode = android.provider.Settings.Global.getInt(
                    ...
                    android.provider.Settings.Global.PREFERRED_NETWORK_MODE,
                    preferredNetworkMode);
            int settingsNetworkMode2 = android.provider.Settings.Global.getInt(
                    mPhone.getContext().getContentResolver(),
                    android.provider.Settings.Global.PREFERRED_NETWORK2_MODE,
                    preferredNetworkMode);
    ...
    public boolean onPreferenceChange(Preference preference, Object objValue) {
        if (preference == mButtonPreferredNetworkMode) {
            ...
            if (TelephonyConstants.IS_DSDS && (DualPhoneController.getdataSim() == 1 ))
                  settingsNetworkMode = android.provider.Settings.Global.getInt(
                    mPhone.getContext().getContentResolver(),
                    android.provider.Settings.Global.PREFERRED_NETWORK2_MODE, preferredNetworkMode);
            if (DBG) log("onPreferenceChange mButtonPreferredNetworkMode buttonNetworkMode: " + buttonNetworkMode);
            if (buttonNetworkMode != settingsNetworkMode) {
                ...
                android.provider.Settings.Global.putInt(mPhone.getContext().getContentResolver(),
                          ...
                          phone2.setPreferredNetworkType(modemNetworkMode,
                          m3gHandler.obtainMessage(My3gHandler.MESSAGE_SET_PREFERRED_NETWORK_TYPE));
                      }
...
//                mButtonPreferredNetworkMode.setEnabled(false);
            ...
            int settingsNetworkMode = android.provider.Settings.Global.getInt(
                    mPhone.getContext().getContentResolver(),
                    android.provider.Settings.Global.PREFERRED_NETWORK_MODE, preferredNetworkMode);
            if (DBG) log("PREFERRED_NETWORK_MODE: " + settingsNetworkMode);
            if (TelephonyConstants.IS_DSDS && (DualPhoneController.getdataSim() == 1 ))
                  settingsNetworkMode = android.provider.Settings.Global.getInt(
                    mPhone.getContext().getContentResolver(),
                    android.provider.Settings.Global.PREFERRED_NETWORK2_MODE, preferredNetworkMode);
            if (DBG) log("PREFERRED_NETWORK2_MODE: " + settingsNetworkMode);
            if (buttonNetworkMode != settingsNetworkMode) {
                int modemNetworkMode;
                // if new mode is invalid ignore it
                switch (buttonNetworkMode) {
        ...
        switch (NetworkMode) {
            case Phone.NT_MODE_WCDMA_ONLY:
                 if (!mIsGlobalCdma) {
                    ...
                    mButtonEnabledNetworks.setSummary(R.string.preferred_network_mode_wcdma_only_summary);
                }
                break;
            case Phone.NT_MODE_GSM_UMTS:
            case Phone.NT_MODE_WCDMA_PREF:
                if (!mIsGlobalCdma) {
                    ...
                    mButtonEnabledNetworks.setSummary(R.string.preferred_network_mode_wcdma_perf_summary);
                } else {
                    mButtonEnabledNetworks.setValue(
            ...
            case Phone.NT_MODE_GSM_ONLY:
                if (!mIsGlobalCdma) {
                    ...
                    mButtonEnabledNetworks.setSummary(R.string.preferred_network_mode_gsm_only_summary);
                } else {
                    mButtonEnabledNetworks.setValue(
                            Integer.toString(Phone.NT_MODE_LTE_CMDA_EVDO_GSM_WCDMA));
                    mButtonEnabledNetworks.setSummary(R.string.network_global);
